...
public class KakaoService {
    ...
    public JwtToken signIn(Map<String,Object> userInfo) {
        Member member = memberRepository.findByUsername(userInfo.get("nickname").toString()+userInfo.get("id").toString())
                .orElseGet(() -> {
                    // └── [없음] → 회원 등록 후 로그인 진행
                    log.info("소셜 회원가입 진행");
                    return memberRepository.save(Member.builder()
                            ...
                            .username(userInfo.get("nickname").toString()+userInfo.get("id").toString())
        ...
        UserDetails userDetails = socialUserDetailsService.loadUserByUserName(member.getUsername());
